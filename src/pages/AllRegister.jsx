...
function OwnerForm() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await fetch(`${API}/api/auth/upload-owner-image`, { method: 'POST', body: form });

      localStorage.setItem('user', JSON.stringify(data));
      alert('ลงทะเบียนเรียบร้อยแล้ว ยินดีต้อนรับสู่ระบบ!');
    ...
    <div className="ar-form-wrapper">
      ...
      <div className="ar-profile-section">
        ...
        <div className="ar-info-grid">
          ...
          <div className="ar-row-3" style={{alignItems:'flex-end'}}>
            <div className="ar-field">
            ...
            <div className="ar-age-display">{age !== '' ? `อายุ ${age} ปี` : ''}</div>
            <div className="ar-field">
          ...
          <div className="ar-row-3">
            ...
            <div className="ar-field" style={{position:'relative'}}>
              ...
              {showPwHint && (
                <div style={{position:'absolute',top:'100%',left:0,marginTop:4,background:'#fff',border:'0.5px solid #e0d6d0',borderRadius:8,padding:'8px 12px',fontSize:11,color:'#5D3A2E',boxShadow:'0 4px 12px rgba(0,0,0,0.1)',zIndex:10,whiteSpace:'nowrap',lineHeight:1.9}}>
      ...
      <div className="ar-btn-group">
        ...
        <button className="ar-btn-submit ar-owner-btn" onClick={handleSubmit} disabled={submitting}>
...
function SitterForm() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await fetch(`${API}/api/auth/upload-sitter-image/${data.sitterID}`, { method: 'POST', body: form });
  ...
  return (
    <div className="ar-form-wrapper">
      ...
      <div className="ar-two-col">
        ...
        <div>
          <div className="ar-section-box">
            ...
            <div className="ar-profile-row">
              ...
              <div style={{flex:1,display:'flex',flexDirection:'column',gap:8}}>
                ...
                <div className="ar-row-2">
                  <div className="ar-field"><label>เพศ <span className="ar-req">*</span></label><select value={gender} onChange={e => { setGender(e.target.value); setErrors(p => ({...p, gender:''})); }}><option value="">-- เลือก --</option><option value="ชาย">♂ ชาย</option><option value="หญิง">♀ หญิง</option></select>{errors.gender && <p className="ar-error">{errors.gender}</p>}</div>
                  <div className="ar-field"><label>โทรศัพท์ <span className="ar-req">*</span></label><input type="text" value={phone} placeholder="0621234567" maxLength={10} onChange={e => { setPhone(e.target.value); setErrors(p => ({...p, phone:''})); }} />{errors.phone && <p className="ar-error">{errors.phone}</p>}</div>
                </div>
                <div className="ar-row-2" style={{alignItems:'flex-end'}}>
                  <div className="ar-field"><label>วันเกิด <span className="ar-req">*</span></label><input type="date" value={birthDate} max={maxBirthDate} onChange={e => { setBirthDate(e.target.value); setErrors(p => ({...p, birthDate:''})); }} />{errors.birthDate && <p className="ar-error">{errors.birthDate}</p>}</div>
                  <div className="ar-age-display">{age !== '' ? `อายุ ${age} ปี` : ''}</div>
                </div>
              </div>
            </div>
            <div className="ar-field" style={{marginTop:8}}><label>อีเมล <span className="ar-req">*</span></label><input type="email" value={email} placeholder="email@example.com" onChange={e => { setEmail(e.target.value); setErrors(p => ({...p, email:''})); }} />{errors.email && <p className="ar-error">{errors.email}</p>}</div>
            <div className="ar-row-2">
              <div className="ar-field" style={{position:'relative'}}><label>รหัสผ่าน <span className="ar-req">*</span></label><input type="password" value={password} placeholder="8-15 ตัวอักษร" onFocus={() => setShowPwHint(true)} onBlur={() => setShowPwHint(false)} onChange={e => { setPassword(e.target.value); setErrors(p => ({...p, password:''})); }} />{showPwHint && (<div style={{position:'absolute',top:'100%',left:0,marginTop:4,background:'#fff',border:'0.5px solid #e0d6d0',borderRadius:8,padding:'8px 12px',fontSize:11,color:'#1E4D6B',boxShadow:'0 4px 12px rgba(0,0,0,0.1)',zIndex:10,whiteSpace:'nowrap',lineHeight:1.9}}>✅ ตัวอักษรภาษาอังกฤษหรือตัวเลข<br />✅ อักษรพิเศษ [! # _ .] ได้<br />✅ ความยาว 8-15 ตัวอักษร</div>)}{errors.password && <p className="ar-error">{errors.password}</p>}</div>
          ...
          <div className="ar-section-box">
            ...
            <div className="ar-map-label-row">
              <span style={{fontSize:13,color:'#5D3A2E'}}>ตำแหน่งบนแผนที่ <span className="ar-req">*</span></span>
          ...
          <div className="ar-section-box">
            ...
            <div className="ar-field">
              ...
              <div className="ar-qr-upload-box" onClick={() => qrRef.current.click()}>
                {qrImage ? <img src={qrImage} alt="QR" className="ar-qr-preview" /> : <><span style={{fontSize:32,color:'#8D6E63'}}>☁️</span><p style={{fontSize:12,color:'#999',marginTop:4}}>คลิกเพื่ออัพโหลด QR Code</p></>}
              </div>
      ...
      <div className="ar-btn-group">
        <button className="ar-btn-back" onClick={() => navigate('/')}>ย้อนกลับ</button>
        <button className="ar-btn-submit ar-sitter-btn" onClick={handleSubmit} disabled={submitting}>
          {submitting ? 'กำลังลงทะเบียน...' : 'ลงทะเบียน'}
        </button>
      </div>
    </div>
