...
function EditAnnouncement() {
  ...
  useEffect(() => {
    const fetchAnnouncement = async () => {
      try {
        ...
        const data = await res.json();
        if (data.pet?.petID) setSelectedPetID(String(data.pet.petID));
        setCurrentweight(data.currentweight || '');
        setStartdate(data.startdate || '');
        setEnddate(data.enddate || '');
        setCareMorning(data.carePet?.careMorning || false);
        setCareAfternoon(data.carePet?.careAfternoon || false);
        setCareEvening(data.carePet?.careEvening || false);
        setCareNight(data.carePet?.careNight || false);
        setFeedMorning(data.carePet?.feedMorning || false);
        setFeedAfternoon(data.carePet?.feedAfternoon || false);
        setFeedEvening(data.carePet?.feedEvening || false);
        setFeedNight(data.carePet?.feedNight || false);
        setIsFoodPrepared(data.carePet?.isFoodPrepared || false);
        setFoodAmount(data.carePet?.foodAmount || '');
        const hasMedicine = data.carePet?.medicineDetail && data.carePet.medicineDetail !== '';
        setNoMedicine(!hasMedicine);
        setMedicineDetail(data.carePet?.medicineDetail || '');
        setIsCleanService(data.carePet?.isCleanService || false);
        setIsWalkService(data.carePet?.isWalkService || false);
        setAddressNo(data.addressNo || '');
        setStreet(data.street || '');
        const distData = CHIANGMAI_DATA[data.district];
        if (distData) setSubdistricts(distData.subdistricts);
        setDistrict(data.district || '');
        setTimeout(() => setSubdistrict(data.subdistrict || ''), 150);
    ...
    if (announceID) fetchAnnouncement();
  }, [announceID]);
  ...
  const validate = () => {
    ...
    if (!startdate) e.startdate = 'กรุณาเลือกวันที่เริ่มดูแล';
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/announcement/update/${announceID}`, {
        method: 'PUT',
      ...
      <div className="body-row">
        ...
        <div className="main-content">
          <div className="ann-card">
            <div className="ann-page-title">แก้ไขประกาศ</div>
