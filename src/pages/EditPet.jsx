...
function EditPet() {
  ...
  // ดึงประเภทสัตว์เลี้ยง + ข้อมูลเดิม
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [typesRes, petRes] = await Promise.all([
          fetch(`${API}/api/pet/types`),
          fetch(`${API}/api/pet/detail/${petID}`),
        ]);
        ...
        setPetTypeID(petData.petType?.petTypeID?.toString() || '');
        setBreed(petData.breed || '');
        setGender(petData.gender || '');
        setBirthDate(petData.birthDate ? petData.birthDate.substring(0, 10) : '');
        ...
        // วัคซีน
        if (petData.vacRabiesDate) {
          setVaccineRabies(true);
          setVacRabiesDate(petData.vacRabiesDate.substring(0, 10));
        }
        if (petData.vacDhppiDate) {
          setVaccineDHPPi(true);
          setVacDhppiDate(petData.vacDhppiDate.substring(0, 10));
        }
        if (petData.vacFvrcpDate) {
          setVaccineFVRCP(true);
          setVacFvrcpDate(petData.vacFvrcpDate.substring(0, 10));
        }

        // โรคประจำตัว
        const hasD = petData.hasCongenitalDisease === 'มี';
        setHasDisease(hasD);
        setDiseaseDetail(petData.congenitalDiseaseDetail || '');
        setEmergencySymptoms(petData.emergencySymptoms || '');
        setEmergencyContact(petData.emergencyContact || '');

        // พฤติกรรม
        setBehaviorStressAlone(petData.behaviorStressAlone || false);
        setBehaviorFriendly(petData.behaviorFriendly || false);
        setBehaviorFearStranger(petData.behaviorFearStranger || false);
        setBehaviorFearLoudSound(petData.behaviorFearLoudSound || false);
        setBehaviorBarkLoud(petData.behaviorBarkLoud || false);
        setBehaviorDislikeTouch(petData.behaviorDislikeTouch || false);
        setBehaviorBiteScrath(petData.behaviorBiteScrath || false);
        setBehaviorEscapeExpert(petData.behaviorEscapeExpert || false);
        setBehaviorHighEnergy(petData.behaviorHighEnergy || false);
        setBehaviorJumpOnPeople(petData.behaviorJumpOnPeople || false);
        setBehaviorHardControl(petData.behaviorHardControl || false);
  ...
  const validate = () => {
    const e = {};
    if (!petImage && !currentImage) e.petImage = 'กรุณาอัพโหลดรูปสัตว์เลี้ยง';
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/pet/update/${petID}`, {
        method: 'PUT',
      ...
      // อัพเดทรูปถ้ามีการเปลี่ยน
      if (petImageFile) {
        const form = new FormData();
        form.append('petImage', petImageFile);
        await fetch(`${API}/api/pet/update-image/${petID}`, {
          method: 'PUT',
          body: form,
        });
      }
        ...
        <div className="main-content">
          ...
          <div className="addpet-grid">
            ...
            <div className="addpet-card">
              ...
              />
              <button className="addpet-upload-btn" onClick={() => fileRef.current.click()}>
                เลือกไฟล์
              </button>
              ...
              {petFileName && (
                <div className="addpet-filename">
                  ...
                  <button onClick={() => {
                    setPetImage(currentImage || null);
                    setPetImageFile(null);
                    setPetFileName(currentImage ? currentImage.split('/').pop() : '');
              ...
              <div className="addpet-field-row">
                <div className="addpet-field">
                  ...
                  <input
                    type="text"
                    value={petName}
                    onChange={e => { setPetName(e.target.value); setErrors(p => ({ ...p, petName: '' })); }}
                ...
                <div className="addpet-field">
                  ...
                  <select
                    value={petTypeID}
                    onChange={e => { setPetTypeID(e.target.value); setErrors(p => ({ ...p, petTypeID: '' })); }}
                  >
              ...
              <div className="addpet-field">
                ...
                <div className="addpet-vaccine-item">
                  <label className="addpet-checkbox-item">
                    <input
                      type="checkbox"
                      checked={vaccineRabies}
                      onChange={e => {
                        setVaccineRabies(e.target.checked);
                        if (!e.target.checked) setVacRabiesDate('');
                        setErrors(p => ({ ...p, vacRabiesDate: '' }));
                      }}
                    />
                    พิษสุนัขบ้า (Rabies)
                  </label>
                  {vaccineRabies && (
                    <div className="addpet-vaccine-date">
                      ...
                      <input
                        ...
                        value={vacRabiesDate}
                        max={today}
                        onChange={e => { setVacRabiesDate(e.target.value); setErrors(p => ({ ...p, vacRabiesDate: '' })); }}
                      />
                ...
                <div className="addpet-vaccine-item">
                  <label className="addpet-checkbox-item">
                    <input
                      type="checkbox"
                      checked={vaccineDHPPi}
                      onChange={e => {
                        setVaccineDHPPi(e.target.checked);
                        if (!e.target.checked) setVacDhppiDate('');
                  ...
                  {vaccineDHPPi && (
                    <div className="addpet-vaccine-date">
                      ...
                      <input
                        ...
                        value={vacDhppiDate}
                        max={today}
                        onChange={e => { setVacDhppiDate(e.target.value); setErrors(p => ({ ...p, vacDhppiDate: '' })); }}
                      />
                ...
                <div className="addpet-vaccine-item">
                  <label className="addpet-checkbox-item">
                    <input
                      type="checkbox"
                      checked={vaccineFVRCP}
                      onChange={e => {
                        setVaccineFVRCP(e.target.checked);
                        if (!e.target.checked) setVacFvrcpDate('');
                  ...
                  {vaccineFVRCP && (
                    <div className="addpet-vaccine-date">
                      ...
                      <input
                        ...
                        value={vacFvrcpDate}
                        max={today}
                        onChange={e => { setVacFvrcpDate(e.target.value); setErrors(p => ({ ...p, vacFvrcpDate: '' })); }}
                      />
                  ...
                  <div className="addpet-disease-box">
                    <div>
                      ...
                      <input
                        type="text"
                        value={diseaseDetail}
                        onChange={e => { setDiseaseDetail(e.target.value); setErrors(p => ({ ...p, diseaseDetail: '' })); }}
                        placeholder="กรอกรายละเอียดโรคประจำตัว"
                      />
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder="กรอกอาการที่ต้องรีบพาไปหาหมอ"
                        style={{ height: 50 }}
                      />
                    </div>
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder="กรอกชื่อโรงพยาบาลหรือเบอร์ติดต่อ"
                        style={{ height: 50 }}
                      />
                    </div>
                  </div>
            ...
            <div className="addpet-card">
              ...
              <div className="addpet-behavior-group">
                ...
                <div className="addpet-checkbox-list">
                  <label className="addpet-checkbox-item">
                    <input type="checkbox" checked={behaviorStressAlone} onChange={e => setBehaviorStressAlone(e.target.checked)} />
                    เครียดเมื่อเจ้าของไม่อยู่
                  </label>
                  <label className="addpet-checkbox-item">
                    <input type="checkbox" checked={behaviorFriendly} onChange={e => setBehaviorFriendly(e.target.checked)} />
                    เฟรนลี่ / ติดคน (ชอบเข้าหาคนตลอด)
                  </label>
                  <label className="addpet-checkbox-item">
                    <input type="checkbox" checked={behaviorFearStranger} onChange={e => setBehaviorFearStranger(e.target.checked)} />
                    กลัวคนแปลกหน้า (ต้องรอปรับตัวเล็กน้อย)
                  </label>
                </div>
              </div>
              ...
              <div className="addpet-behavior-group">
                ...
                <div className="addpet-checkbox-list">
                  <label className="addpet-checkbox-item">
                    <input type="checkbox" checked={behaviorFearLoudSound} onChange={e => setBehaviorFearLoudSound(e.target.checked)} />
                    กลัวเสียงดัง (ฟ้าร้อง / เครื่องใช้ไฟฟ้า)
                  </label>
                  <label className="addpet-checkbox-item">
                    <input type="checkbox" checked={behaviorBarkLoud} onChange={e => setBehaviorBarkLoud(e.target.checked)} />
                    เห่า/ส่งเสียงดังเมื่อมีสิ่งกระตุ้น
                  </label>
                </div>
              </div>
              ...
              <div className="addpet-behavior-group">
                ...
                <div className="addpet-checkbox-list">
                  <label className="addpet-checkbox-item">
                    <input type="checkbox" checked={behaviorDislikeTouch} onChange={e => setBehaviorDislikeTouch(e.target.checked)} />
                    ไม่ชอบให้จับบางจุด (เช่น หู ท้อง ขา)
                  </label>
                  <label className="addpet-checkbox-item">
                    <input type="checkbox" checked={behaviorBiteScrath} onChange={e => setBehaviorBiteScrath(e.target.checked)} />
                    กัดหรือข่วนเมื่อเครียด
                  </label>
                  <label className="addpet-checkbox-item">
                    <input type="checkbox" checked={behaviorEscapeExpert} onChange={e => setBehaviorEscapeExpert(e.target.checked)} />
                    หนีเก่งเมื่อเปิดประตู
                  </label>
                  <label className="addpet-checkbox-item">
                    <input type="checkbox" checked={behaviorHighEnergy} onChange={e => setBehaviorHighEnergy(e.target.checked)} />
                    เล่นแรง / พลังงานสูง
                  </label>
                  <label className="addpet-checkbox-item">
                    <input type="checkbox" checked={behaviorJumpOnPeople} onChange={e => setBehaviorJumpOnPeople(e.target.checked)} />
                    กระโดดใส่คนหรือสิ่งของ
                  </label>
                  <label className="addpet-checkbox-item">
                    <input type="checkbox" checked={behaviorHardControl} onChange={e => setBehaviorHardControl(e.target.checked)} />
                    ควบคุมยากเมื่อเครียด
                  </label>
                </div>
              </div>
            </div>
